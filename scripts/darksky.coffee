          ...
          darkSkyMe msg, lat,lng , options.separator, (darkSkyText) ->
            response = "Weather for #{result.results[0].formatted_address}#{options.separator}#{darkSkyText}"
              .replace /-?(\d+\.?\d*)°C/g, (match) ->
                ...
                match = "#{Math.round( centigrade * ( 9 / 5 ) + parseInt( 32, 10 ) )}°F"
...
darkSkyMe = (msg, lat, lng, separator, cb) ->
  ...
  msg.http(url)
    .get() (err, res, body) ->
      ...
      response += "#{separator}For the hour: #{result.minutely.summary}"
      response += "#{separator}Next 24 hours: #{result.hourly.summary}"
      response += "#{separator}Week: #{result.daily.summary}"
